...
export function calcStreak(history) {
  ...
  const checkFrom = history[today] ? today : (history[yesterday] ? yesterday : null);
  if (!checkFrom) return 0;
  let cur = new Date(checkFrom + 'T00:00:00');
  while (true) {
...
export function calcRate(history, days = 30) {
  ...
  return Math.round((relevant.filter(([, v]) => v).length / relevant.length) * 100);
}
